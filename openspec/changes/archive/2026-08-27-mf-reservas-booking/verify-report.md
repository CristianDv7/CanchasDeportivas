...
**WARNING** (should fix):
1. **Corrección realizada al archivar**: el escenario de `specs/mf-reservas-backend-adapter/spec.md` ("canchasApi mapea disponibilidad") fue ajustado para claridad: se especifica que solo `GET /reservas/disponibilidad` (contrato propuesto entregado por backend) es el endpoint real en uso, eliminando la mención hipotética de `/horarios-atencion` que aparecía como plan B no implementado.
2. El conteo de tareas mencionado en la invocación (58 tareas) no coincide con `tasks.md` real (36 tareas `[x]` en 10 fases). No afecta el veredicto — 100% de las tareas reales están completas.
