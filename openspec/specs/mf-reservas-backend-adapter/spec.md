...
#### Scenario: reservasApi arma disponibilidad combinando dos fuentes

- GIVEN una respuesta cruda `list[HorarioAtencionResponse]` de `GET /horarios-atencion` (ms-canchas, vía `canchasApi.getHorariosAtencion()`) y una respuesta cruda `list[ReservaResponse]` (solo `Confirmada`) de `GET /reservas/disponibilidad` (ms-reservas) — este es el contrato REAL implementado por el backend, no una grilla ya armada
- WHEN `reservasApi.getDisponibilidad()` las procesa
- THEN devuelve un `Disponibilidad` DTO en camelCase con `BloqueDisponibilidad[]` de 1h generados por el adapter (`buildDisponibilidad`, `src/api/mappers.ts`), marcando `ocupado` cada bloque que se solapa con una reserva confirmada, sin exponer `usuarioId` ni ningún otro campo crudo de la reserva

Nota (2026-08-27): el contrato originalmente asumido para `GET /reservas/disponibilidad` (una grilla `{cancha_id, fecha, bloques}` ya armada por el backend) nunca se implementó así — el endpoint real devuelve `list[ReservaResponse]` sin grilla, y `GET /horarios-atencion` (ya documentado como gap pendiente) pasó a ser una dependencia real del adapter para poder armarla client-side.
