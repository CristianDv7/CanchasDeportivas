...
Los DTOs del frontend mantendrán fecha/hora como `string`, sin parsearlas a `Date` al cruzar la frontera API → DTO. El parseo (u operación equivalente) ocurrirá recién en el punto de uso — específicamente en la capa de Reglas de Dominio, donde vive la lógica de RN-04.
...
- La capa de Reglas de Dominio queda como el único lugar que decide cómo se interpreta el tiempo — testeable de forma aislada y pura (ver [ADR-12](ADR-12-reglas-negocio-cliente-solo-computable.md)).
...
Fecha/hora se mantienen como `string` hasta el único punto de uso que las interpreta, evitando el bug clásico de parsear `Date` tempranamente en JS.
