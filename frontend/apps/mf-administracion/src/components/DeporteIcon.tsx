// Ícono por deporte para las tarjetas de cancha (Canchas/CanchasPage). Trazo
// simple en --turf, coherente con la estética tipográfica/flat del resto de
// la app — no hay pipeline de assets fotográficos en el repo, así que un SVG
// inline por deporte cubre "más vistoso" sin sumar una dependencia externa.
...
export function DeporteIcon({ deporte }: DeporteIconProps) {
  return (
    <span className="mfa-deporte-icon" data-testid="deporte-icon" title={deporte ?? undefined}>
      {iconFor(deporte)}
    </span>
  );
}
