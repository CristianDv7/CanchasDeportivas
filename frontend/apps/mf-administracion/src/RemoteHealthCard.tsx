...
// (design.md dice "sin paquete compartido": se copia, no se extrae).
...
export function RemoteHealthCard() {
  ...
  if (forceError) {
    throw new Error("Error forzado desde RemoteHealthCard (mf-administracion)");
  }

  async function checkBackend(): Promise<void> {
    setProbeStatus("checking");
    try {
      await apiClient.get("/", { service: "canchas" });
