// RED: design.md §3 — el banner solo renderiza `message` y condiciona el
// botón "Reintentar" a `action === "retry"`. No ramifica por `status`.
...
describe("ErrorBanner", () => {
  it("muestra el message dentro de un role=alert", () => {
    const error: UiError = { message: "Ya existe una reserva en ese horario.", action: "none", status: 400 };
  ...
  it("no muestra el botón Reintentar si action !== 'retry'", () => {
    const error: UiError = { message: "No tenés permiso para operar sobre esta reserva.", action: "none", status: 403 };
